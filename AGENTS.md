...
pie 是一个极简的 agent harness，**纯 Rust 实现**（2026-09-23 从旧的纯 Python 版重构而来，Python 代码已从本仓库删除；那段历史与两版差异见 `docs/python-legacy.md`）：
...
├── docs/             # CHANGELOG.md（历史决策）/ python-bindings.md（绑定规划与进度）/ python-legacy.md（旧 Python 版历史与差异）
├── AGENTS.md         # 本文件（运行时被拼进 system prompt）
├── MEMORY.md         # 项目持久记忆（同上；要查历史决策看 docs/CHANGELOG.md）
└── README.md         # 面向人的总说明：进度表 / 工具怎么写 / 用法 / 与旧 Python 版的差异
```
...
- **输出协议**：`Headers\n\nBody`——headers 一行一个 `[key=value]`，body 为空时连空行都不给。
  ...
  （`[exit=0]` 只来自旧会话 / Python 版，也得认）；那行没有「纯值」约束，别写 `^\[exit=(\d+)\]$`。
...
- **JSONL 落盘要转义「行分隔类」字符**（`session::json_line` / `escape_control_chars`）：serde_json 只转义 C0，
  ...
  **U+0085 当换行**，一条消息被劈成两半、整份会话读不出来（工具输出里出现这些字节一点不稀奇）。
  session 文件与 `/clear` 的窗口块（`~/.pie/windows/*.jsonl`）都走 `json_line`。
...
- **失败也要留一条 assistant**：模型请求出错（网络 / 服务端 / 协议）时不直接 `return Err`，
  ...
  不补就留下「没人应答的提问」（历史里连续两条 user、resume 回放也莫名其妙）。
  调用方仍拿 `Err`（界面照旧报错），只是历史里 user/assistant 始终成对。
  CLI 的会话模式（`-r` / `-s` + 任务）在 `Err` 上**也要 `save()`** 再 `return 1`
  （否则这轮提问连同失败原因一起消失）；TUI 本来就是退出时一定存。
...
- **告警不能直接写 stderr**：raw mode + 交替屏下写 stderr 的字节落在当前光标处，而 ratatui 只重画
  ...
  没装就 `eprintln!`）。
- **聚焦才亮**的几档颜色只在**色板**里定义一次（`Palette::style_emphasis` / `style_caret` / `style_selection`）：
  `tui::run` 开了 `EnableFocusChange` → `Event::FocusGained/FocusLost` 进 `App::focused`（初值 `true`：
  多数终端根本不发焦点事件，收不到就当聚焦），再一路传给各视图的渲染。
  - `style_emphasis(focused)` = 需要注意力的一档：聚焦 accent / **失焦 muted**（输入框上边框、状态栏名字、
    spinner 与耗时都走它；输入框的 `!cmd` 分支在聚焦时改工具色，失焦仍回 muted）；
  - `style_caret(focused)` = 光标块：聚焦 = `style_selection()`（accent 底 + accent_text 字），失焦 = muted 底
    ——**不能**用控件默认的裸 `REVERSED`：那颜色会跟着所在行跑，空输入时叠上 placeholder 的 muted 行样式
    就变成一块暗灰（看着像没光标）；`style_selection()` 同时供输入框选区与消息流框选（`App::paint_selection`）。
  - 所以「窗口在不在前台」一眼就看得出来（输入框 + 状态栏一起变灰），而各视图**不该**再自己写一遍「失焦就 muted」。
- **宽字符的后半格得自己擦**：`BufferDiff` 不重画宽字形后面那格（`set_stringn` 会把它 `reset()` 成普通
  空格，而 `Cell::eq` 与真空格全等）→ 删字后留下“半个汉字”/底色方块、placeholder 里的 `⏎`/`⇧` 也会
  留碎片。`Input::render` 每帧用 `StaleTail` 把「已写区间右边、上一帧写过的那一段」标
  `CellDiffOption::AlwaysUpdate` 强制重画：只在**行变短**那帧标（`AlwaysUpdate` 只对当帧有效，静止帧
  零开销），且**只标已写区间之外**——写正文里那格会把汉字擦掉半个（光标压在宽字符上时是 2 格色块，
  那是控件把光标样式涂在字形那格上，与选区同款，**有意保留**）。
- **终端光标位置 = 输入法的锚点**：光标块是 TUI 自己画的、终端光标一直隐藏，但每帧 `App::run`
  在 `terminal.draw` **之后**还要 `terminal.set_cursor_position(插入符)`（位置来自 `Input::caret_position`）。
  IME 的候选框跟的是**终端光标单元格**（VS Code 的 xterm 把隐藏 textarea 摆在终端光标处、输入法起手
  还会再同步一次），不摆位置它就停在「上一帧 diff 最后写入的那一格」——点一下消息流（只为选中高亮
  重画被点的那一格）候选框就会跑到点击处。
...
- 记忆分工：项目决策/踩坑写本仓 `MEMORY.md`，历史与来龙去脉写 `docs/CHANGELOG.md`，
  跨项目习惯写 `~/.pie/memory.md`，旧 Python 版的历史与两版差异写 `docs/python-legacy.md`。
  与代码冲突时**以代码为准**，过时条目就地改或删。
...
- `Config.theme` **已被读取**（2026-09-24）：TUI 启动时 `Palette::resolve(&config.theme)` → 认 `catppuccin`（族名）/ `catppuccin-<flavor>` / 裸 flavor 名（大小写、空白、重音宽容），认不出回 Mocha 并在消息流提一句 `[theme] …`。**仍缺**明暗自适应（无 OSC 11 探测 → 族名按深色）与 `/theme` 运行中热切（`Palette` 现在仍是启动时定下，`markdown` 缓存不随色板变）。
