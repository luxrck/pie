...
默认 TUI 给每条消息套盒子（user / assistant / 工具调用 / 工具结果）。开启简洁模式后**只有 user 与 assistant 消息套盒子**，工具活动压成单行，适合长时间工具循环时保持对话可读：

```
✧ read src/pie/tui.py                    ← 工具调用：icon 工具名 摘要
↳ read src/pie/tui.py ✅                 ← 工具结果：摘要 + 状态标记
❯ shell ls /nope
↳ shell ls /nope ❌
  → ls: cannot access '/nope': No such file or directory   ← 只有出错才缩进输出正文
```

摘要取哪个参数：read / write / edit → 文件 `path`，shell → `command`，其余工具退回完整参数 JSON；状态标记 ✅ 成功 / ❌ 失败（shell 非 0 退出码、`[工具错误]` 等失败前缀）/ ⏹ 被 `/stop` 终止。**失败行整行染红**（图标 / 工具名 / 摘要 / ❌ 与下方错误正文同色，一眼看出是哪次调用挂了）；调用行与成功行保持橙 / 棕 + 次级色摘要。摘要里的**换行会转写成字面 `\n`**（多行 shell 命令 / heredoc 也只占一行）、tab 转成空格，所以工具行不会被内容拆成多行。错误正文最多显示 12 行开头 + 7 行结尾（中间省略并标注行数），避免长输出刷屏；`!cmd` 是用户主动执行，输出总是显示（额度 50 + 20 行）。

工具行**恒占一 display 行**：摘要超出可用宽度时在渲染时按宽度截断成 `…`（先截摘要、后放标记，行尾 ✅/❌ 优先保留），所以再长的 shell 命令也不会把对话撑成多行。需要截断的行会额外留出 1 列右边距，行尾标记不会贴到边框上。同一批并行工具调用：调用行先按调用顺序全部输出（能立即看到在跑什么），结果行按**实际完成顺序**输出；每行都自带工具名 + 摘要（实时事件带 arguments、resume 用 tool_call_id 配对），所以不会串位；resume 回放则按会话文件里的调用顺序。

在 `~/.pie/config.toml` 里配置：
...
[tui]
# 简洁模式：只有 user / assistant 套盒子，工具调用/结果压成单行（见「简洁模式」一节）
lean = false
```
