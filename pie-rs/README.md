...
| `llm` | ✅ | **reqwest + 手写 SSE**，不含任何 OpenAI SDK；自研重试（`Retry-After` 优先） |
...
| CLI | ✅ | 一次性模式 `pie-rs "任务"`（支持 stdin）；`-m/-t/--reserved-tokens/--max-steps/--no-stream/--cwd/--stat/--tools/--system-prompt` 等覆盖项；`-r/--resume`、`-s/--session`；子命令 `sessions`/`context`/`files` |
...
不同**（Python 也如此）：`read` 的行不含换行 → 每行 `len+1`；`shell` 的行含 `\n` → 按实际字节（各自就地实现）。
...
`generated_specs_match_python`：工具定义与 Python 版逐字对比（基准 `fixtures/python-tools.json`）。
唯一放过的差异是 `required` 的**元素顺序**（schemars 字母序 / Python 声明序）——JSON Schema 里
`required` 是集合，所以比较前两边都排序。改了 schema 就重新生成基准：
...
cargo run -- --reserved-tokens 64k --stat "任务"  # 覆盖输出预留，跑完看 /stat
...
cargo run -- --stat "任务"                     # 跑完打印 /stat 报告（上下文占用 / 水位 / 压缩事件 / API 用量）
...
**有意为之**
- **内置提示词正文在 `prompts/`（小写文件名）**：`prompts/system.md`（= `DEFAULT_SYSTEM_PROMPT`，与 Python 版
  `config.SYSTEM_PROMPT` 常量逐字一致）、`prompts/memory.md`（= `DEFAULT_GLOBAL_MEMORY`，全局记忆种子）。
...
- **记忆文件与 Python 共用同一批路径**：`~/.pie/memory.md`（全局）+ 项目根的 `MEMORY.md`/`AGENTS.md`/`SYSTEM.md`。
  ...
  `edit`/`write` 维护；system prompt 里按 `## 全局记忆（<绝对路径>）` + `## 项目记忆（MEMORY.md）` 拼
...
- **上下文三级压缩**（`context.rs`，2026-09-22）：语义照 Python——工具级（head/tail 预览 + `[工具输出全文已保存: …]`
  ...
  另外：manifest 的 `ts` 是自算的 ISO UTC（没有日期库）、落盘原文用 `indent=1` 的 JSON（对齐 Python
  `json.dumps(..., indent=1)`）、会话级窗口块落在 `~/.pie/context/session-*.txt`（Python 的
  `~/.pie/windows/` 是 `/clear` 归档用的，Rust 还没实现 `/clear`）。
...
- **`max_steps` / `stream` 收进配置**（2026-09-22）：Python 里它们是 `loop.aturn` 的形参（由嵌入方按次传，
  `None` = 跟随后端能力），Rust 改成 `Config.max_steps`（`Option<usize>`，不写 = 不限）与 `Config.stream`
  （默认 `true`）——`run_turn` 因此少两个形参（8 → 6）。要按次覆盖就在调用前改一下 cfg 字段。
  连带补上了 `TurnEvent::Answer`：**只在非流式（`stream = false`）时推**，因为流式下正文已经走
  `AssistantText` 增量推过了、再推会重复显示（Python 是流式也推 `answer`，靠 TUI 覆盖面板绕过）。
...
- **消息流框选复制**（2026-09-23）：鼠标捕获开着（滚轮要用）→ 终端原生选择用不了，所以自己做：
  ...
  （`clipboard::copy_text`，与 Python 同款：复制完清选区 + 右下角弹一条 Toast），
...
**还没做（TODO，按优先级）**
- `parallel_tools`：目前工具**串行**执行（Python 版默认并发；`Config.parallel_tools` 已在）。
...
- TUI 细化：代码块语法高亮、`/clear` 切窗口（窗口归档）。（`!cmd` 手动 shell、resume
  历史回放、消息流框选复制已于 2026-09-23 接上。）
