...
| CLI | ✅ | 一次性模式 `pie-rs "任务"`（支持 stdin）；`-m/-t/--reserved-tokens/--mode/--max-steps/--no-stream/--cwd/--stat/--tools/--system-prompt` 等覆盖项；`-r/--resume`、`-s/--session`；子命令 `sessions`/`context`/`files` |
