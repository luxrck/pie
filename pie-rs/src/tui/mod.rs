//! TUI：ratatui + crossterm 的交互界面（**参考 codex 的形态**：顶栏 + 消息流 + 底部输入区）。
//!
...
//!   - `status`：顶栏（模型、上下文占用、**思考计时**/spinner）与底栏提示
...
pub async fn run(session: Session) -> std::io::Result<()> {
    ...
    let (app, rx) = app::App::new(session);
